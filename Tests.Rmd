...
DF <- data.frame(AM = AM, 
                 CI.lo = c(CI$female$conf.int[1], 
                           CI$male$conf.int[1]),
                 CI.up = c(CI$female$conf.int[2], 
                           CI$male$conf.int[2]),
                 sex = c("female", "male"),
                 n = n)
...
```{r}
## pairwise Welch t-tests without patient 398
pairwise.ext.t.test(ICUData$temperature[-398], ICUData$outcome[-398])
## pairwise Wilcoxon-Mann-Whitney U tests
pairwise.wilcox.exact(ICUData$temperature, ICUData$outcome)
```
...
DF <- data.frame(AM = AM, 
                 CI.lo = c(CI$died$conf.int[1], 
                           CI$`home`$conf.int[1],
                           CI$`other hospital`$conf.int[1],
                           CI$`secondary care/rehab`$conf.int[1]),
                 CI.up = c(CI$died$conf.int[2], 
                           CI$`home`$conf.int[2],
                           CI$`other hospital`$conf.int[2],
...
```{r}
## pairwise paired t-tests without patient 398
pairwise.ext.t.test(SE.long$score, SE.long$time, paired = TRUE)
## pairwise Wilcoxon signed rank tests
pairwise.wilcox.exact(SE.long$score, SE.long$time, paired = TRUE)
```
